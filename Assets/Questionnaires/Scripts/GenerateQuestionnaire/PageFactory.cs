    ...
    public class PageFactory : MonoBehaviour
    {
        ...
        */
        ...
        public void InitSetup()
        {
            ...
            nextButton[0].gameObject.SetActive(false);
        }
            ...
            switch (qType)
            {
                case "radio":
                    if (_qData.Count < QuestionPerPage)
                    {
                        for (int i = 0; i < _qData.Count; i++)
                        {
                            ...
                            var radioHorizontalLayout = radioHorizontalRec.gameObject.transform.GetChild(1);
                            var radioHorizontalLayoutRec = radioHorizontalLayout.gameObject.GetComponent<RectTransform>();
                            ...
                            SetRec(radioHorizontalLayoutRec);

                            QuestionList.Add(temp.GetComponent<Radio>().CreateRadioQuestion(qId, qType, qInstructions, _qData[i][0], _qData[i][1], _qData[i][2].AsBool, _qData[i][3].AsArray, i, radioHorizontalLayoutRec));
                        }
                    }
                    else
                    {
                        Debug.LogError("We currently only support up to 5 radio questions per page");
                    }
                    break;
                case "radioGrid":
                    if (qConditions.Count < 8)
                    {
                        for (int i = 0; i < qConditions.Count; i++)
                        {
                            ...
                            var radioGridHorizontal = radioHorizontalRec.gameObject.transform.GetChild(1);
                            var radioGridHorizontalRec = radioGridHorizontal.gameObject.GetComponent<RectTransform>();
                            ...
                            QuestionList.Add(temp.GetComponent<RadioGrid>().CreateRadioGridQuestion(qId, qType, qInstructions, _qData[0][0],
                                _qData[0][1], _qData[0][2].AsBool, qOptions, qConditions[i][1], i, radioGridHorizontalRec));
                        }
                    }
                    else
                    {
                ...
                case "checkbox":
                    ...
                    for (int i = 0; i < _qData.Count; i++)
                    {
                        ...
                        var radioGridHorizontal = radioHorizontalRec.gameObject.transform.GetChild(0);
                        var radioGridHorizontalRec = radioGridHorizontal.gameObject.GetComponent<RectTransform>();
                ...
                case "dropdown":
                    for (int i = 0; i < _qData.Count; i++)
                    {
                        ...
                        QuestionList.Add(temp.GetComponent<Dropdown>().CreateDropdownQuestion(qId, qType, qInstructions, _qData[i][0], _qData[i][1], _qData[i][2].AsArray, i, radioGridHorizontalRec));
                    }
                    break;
                case "textInput":
                    for (int i = 0; i < _qData.Count; i++)
